...
static void init(void)  {
    ...
    model_init();
#if FFMPEG
    ffmpeg_encoder_start("/tmp/offscreen/tmp.mpg", AV_CODEC_ID_MPEG1VIDEO, 25, width, height);
#endif
}
...
static void display(void) {
    ...
    if (output_formats & PPM_BIT) {
        snprintf(filename, SCREENSHOT_MAX_FILENAME, "/tmp/offscreen/tmp.%d.ppm", nframes);
        screenshot_ppm(filename, width, height, &pixels);
    }
#endif
#if LIBPNG
    if (output_formats & LIBPNG_BIT) {
        snprintf(filename, SCREENSHOT_MAX_FILENAME, "/tmp/offscreen/tmp.%d.png", nframes);
